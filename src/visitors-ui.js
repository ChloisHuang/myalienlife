...
export function createVisitorsUi(client){
 const button=document.createElement('button');button.id='visitor-stats';button.hidden=true;button.title='访问统计';button.setAttribute('aria-label','访问统计');button.append(createElement(ChartColumn));document.querySelector('.online-controls').append(button);
 const dialog=document.createElement('dialog');dialog.id='visitor-dialog';dialog.setAttribute('aria-labelledby','visitor-title');
 dialog.innerHTML='<header><h2 id="visitor-title">访问统计</h2><button aria-label="关闭访问统计" title="关闭"></button></header><p id="visitor-message" role="status"></p><div id="visitor-results" hidden><dl class="visitor-totals"><div><dt>总访问人次</dt><dd id="visitor-total"></dd></div><div><dt>今日独立 IP</dt><dd id="visitor-today"></dd></div><div><dt>近 30 天访问人次</dt><dd id="visitor-month"></dd></div></dl><h3>近 30 天访问人数</h3><div class="visitor-chart"><canvas role="img" aria-label="近 30 天每日独立 IP 曲线"></canvas></div><h3>IP 归属地</h3><div class="visitor-countries"><table><thead><tr><th>国家 / 地区</th><th>访问人次</th><th>占比</th></tr></thead><tbody></tbody></table></div><p class="visitor-note" id="visitor-since"></p><p class="visitor-note">北京时间 · 同一 IP 连续访问计一次，闲置 30 分钟后再访另计。人数按每日独立 IP 估算；共享网络与代理可能影响结果。仅保存匿名标识和国家 / 地区，不保存原始 IP。</p></div>';
 ...
 button.onclick=async()=>{
  ...
  try{
   ...
   set('#visitor-total',data.totalVisits);set('#visitor-today',data.days.at(-1).visitors);set('#visitor-month',data.days.reduce((sum,d)=>sum+d.visits,0));
