...
export function createFloatingIsland(scene,{returnIcon,onEnter,onLeave,onWindowChange}){
 const host=scene.parentNode,anchor=document.createComment('island viewport');scene.before(anchor);
 ...
 function restore(){
  if(!floating)return;
  floating=null;anchor.after(scene);host.inert=false;document.body.classList.remove('island-floating');pageReturn.hidden=true;onLeave();onWindowChange(window);
 }
 ...
 return {
  async open(){
   ...
   try{
    ...
    onEnter();popup.document.body.append(scene,returnButton);host.inert=true;document.body.classList.add('island-floating');pageReturn.hidden=false;onWindowChange(popup);
