...
export const STAR_ISLANDS={
 eva:{name:'露米纳星湾',level:0,color:0xbce0d4,skill:null,required:0,interests:[]},
...
export const populationCapacity=g=>Object.keys(islandCatalog(g)).filter(id=>id==='eva'||g.civilization.visits[id]>0).length*8;
...
export const createCivilization=()=>({seed:crypto.getRandomValues(new Uint32Array(1))[0],islands:{},destroyedIslands:[],projects:{spore:createProject(),ocean:createProject()},discoveryPath:['eva'],knowledge:0,technology:0,observations:0,lastDiscoveryObservation:0,surveys:{spore:0,ocean:0},surveyDays:{spore:0,ocean:0},visits:{eva:1,spore:0,ocean:0}});
...
export function civilizationError(g,type,o,p,skills,id,count=1,actionId=null,shipId=null){
 ...
 if(type==='explore'&&islandOf(p)!=='eva'&&g.civilization.surveyDays[islandOf(p)]===g.day)return '这座星岛今天已经完成勘察。';
 return null;
}
export function contributeCivilization(g,type,p,skills,career){
 const c=g.civilization,id=islandOf(p);
 const knowledge={research:1,observe:1,traceRelic:2,decodeRelic:3,decodeTogether:3,restoreMemory:5,explore:id==='eva'?1:6}[type]??0;c.knowledge+=knowledge;
 ...
 if(type==='explore'&&id!=='eva'){c.surveys[id]++;c.surveyDays[id]=g.day;}
}
export function validCivilization(c){
 const count=n=>Number.isSafeInteger(n)&&n>=0&&n<=1e9;
 if(!c||!Array.isArray(c.destroyedIslands)||new Set(c.destroyedIslands).size!==c.destroyedIslands.length||c.destroyedIslands.some(id=>id==='eva'||!c.discoveryPath?.includes(id))||!validProjects(c))return false;
 const sequence=['eva','spore',...Object.values(c?.islands??{}).sort((a,b)=>a.index-b.index).map(b=>b.id),'ocean'];
