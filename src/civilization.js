...
export function discoverProceduralIslands(c,random=Math.random){
 ...
 const b=generateIsland(c.seed,index);c.islands[b.id]=b;c.visits[b.id]=0;c.surveys[b.id]=0;c.surveyDays[b.id]=0;return b;
}
export const createCivilization=()=>({seed:crypto.getRandomValues(new Uint32Array(1))[0],islands:{},knowledge:0,technology:0,observations:0,lastDiscoveryObservation:0,surveys:{spore:0,city:0},surveyDays:{spore:0,city:0},visits:{home:1,spore:0,city:0}});
...
export function contributeCivilization(g,type,p,skills,career,random=Math.random){
 ...
 if(type==='observe'||type==='explore'&&id==='home'){c.observations++;discoverProceduralIslands(c,random);}
