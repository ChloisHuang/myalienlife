...
export const workbench=o=>o?.type==='lab'||o?.type==='stove';
...
export function projectError(g,type,o,p){
 ...
 if(!workbench(o))return '请使用任意全息研究台或孢火星釜。';
 if(type==='developBlueprint')return project.blueprint>=PROJECT_WORK.blueprint?'本岛蓝图已开发完成。':null;
 return project.blueprint<PROJECT_WORK.blueprint?'蓝图开发达到 100% 后才能开始建设。':projectComplete(g,id)?'本岛建设已完成。':null;
}
export function contributeProject(g,type,p,seconds){
 ...
 project[key]=Math.min(PROJECT_WORK[key],project[key]+seconds);
}
