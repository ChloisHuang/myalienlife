...
import {refuses} from './living-spatial.js';
...
export const LIVING_ACTIONS={
 ...
 treeRest:{name:'树荫休养',icon:'TreePine',duration:20,effects:{energy:24,comfort:18}},
 seekLight:{name:'靠近曦光休息',icon:'Sun',duration:16,effects:{comfort:20,social:8}},
 witnessPrayer:{name:'陪伴觉醒',icon:'Sparkles',duration:12,effects:{social:14}}
};
...
export function validLivingQueue(g,q){
 ...
 return ['tendTree','treeRest'].includes(q.type)?o?.type==='spiritTree'&&supportedIsland(o):o?.type==='gate'&&inForest(o);
}
...
export function livingError(g,type,p,o,other){
 ...
 if(['tendTree','treeRest'].includes(type))return o?.type==='spiritTree'&&supportedIsland(o)?null:'请选择圣树。';
 ...
 if(type==='seekLight'&&!isRadiant(other))return '这位居民没有曦轮。';
 if(type==='witnessPrayer'&&(other.uid===g.player.uid?g.queue:other.queue)[0]?.type!=='pray')return '对方目前没有进行祈祷。';
 if(['seekLight','accompany','witnessPrayer'].includes(type)&&refuses(g,p,other))return '暂时不愿与对方同行。';
 if(type==='shareLight'&&refuses(g,other,p))return '对方暂时不愿接受你的照顾。';
...
export function finishLiving(g,person,q,o,people,before={}){
 ...
 if(q.type==='seekLight'){state().fear=bounded(state().fear-35);changeBond(g,p,b,{trust:5,dependence:4});}
 if(q.type==='witnessPrayer'){changeBond(g,p,b,{trust:6});changeBond(g,b,p,{trust:8});}
...
export function livingBonus(g,p,c,people){
 ...
 if(c.type==='pray'&&o?.type==='spiritTree'){
  if(livingSite(g,o).vitality<45&&people.some(a=>nearby(o,a.position,6)&&vulnerable(g,a.position)&&a.needs.energy<70))return null;
  if(people.some(a=>a!==p&&a.queue[0]?.targetId===o.id&&(bondTo(g,person,a.position)?.resentment??0)>=20))return null;
 }
 if(c.type==='treeRest')return vulnerable(g,person)||p.needs.energy<65?25:null;
 if(c.type==='seekLight')return s.fear>=25?55+s.fear:null;
 if(c.type==='witnessPrayer')return ['pray'].includes(other?.queue[0]?.type)&&(bondTo(g,person,other.position)?.trust??0)>=20?60:null;
 ...
 if(c.type==='accompany')return s.fear>=40||!other||bondWeight(g,person,other.position)<0||(!(inForest(person)&&livingResident(g,other.position).fear>=20)&&other.queue[0]?.phase!=='walking')?null:20+livingResident(g,other.position).fear*.5+bondWeight(g,person,other.position);
...
export const vulnerable=(g,p)=>p.age<g.config.lifeStages.infantEnd||p.age>=g.config.lifeStages.adultEnd;
...
export function advanceLiving(g,people){
 ...
 for(const o of g.objects)if(o.type==='spiritTree'&&g.living.sites[o.id]){
  const site=g.living.sites[o.id],guardian=local.some(a=>livingResident(g,a.position).imprint==='roots'&&nearby(a.position,o,5));site.vitality=bounded(site.vitality+n*(guardian?2:1));
  for(const a of local)if(vulnerable(g,a.position)&&nearby(a.position,o,3)&&site.vitality>=12){a.needs.comfort=Math.min(100,a.needs.comfort+3*n);a.needs.energy=Math.min(100,a.needs.energy+2*n);site.vitality=bounded(site.vitality-n);}
 }
 for(const a of local){
  const p=a.position,neighbors=(groups.get(`${islandOf(p)}:${sideOf(p)}`)??[]).filter(b=>b!==a&&nearby(p,b.position,3));
  if(isDual(p)&&!a.queue[0]?.blinkTransit){const s=mutableResident(g,p);s.mode=a.queue[0]?.type==='shareLight'||local.some(b=>b.queue[0]?.type==='seekLight'&&b.queue[0].targetId===a.id)||neighbors.some(b=>livingResident(g,b.position).fear>=25)?'light':'shadow';}
 }
