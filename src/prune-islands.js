...
export function pruneGeneratedIslands(g){
 ...
 const evaSpot=()=>{for(let z=2;z<=6;z+=2)for(let x=-8;x<=8;x+=2)if(!g.objects.some(o=>islandOf(o)==='eva'&&o.side==='front'&&Math.hypot(o.x-x,o.z-z)<1.8))return{x,z,island:'eva',side:'front'};throw new Error('主岛没有安置空间');};
 const destination=evaSpot(),cancelled=new Set();
 ...
 let ships=0;for(const ship of g.space.ships){if(removed.has(ship.island)||removed.has(ship.flight?.from.island)||removed.has(ship.flight?.to.island)){ship.island='eva';ship.side='front';delete ship.flight;ships++;}if(cancelled.has(ship.reservedBy))ship.reservedBy=null;}
 // Keep unborn residents and their occupied nursery when removing an island.
 const occupied=new Set(g.incubations.map(b=>b.podId));for(const o of g.objects)if(removedObjects.has(o.id)&&occupied.has(o.id)){Object.assign(o,evaSpot());removedObjects.delete(o.id);}
 g.objects=g.objects.filter(o=>!removedObjects.has(o.id));
 for(const id of removed){g.space.provisions.eva=(g.space.provisions.eva??0)+(g.space.provisions[id]??0);delete g.space.provisions[id];delete g.space.backs[id];for(const key of ['islands','visits','surveys','surveyDays'])delete g.civilization[key][id];}
 g.civilization.lastDiscoveryObservation=g.civilization.observations;
 if(removed.has(g.viewIsland)){g.viewIsland='eva';g.viewSide='front';}
