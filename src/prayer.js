...
const prayerCaps={radiance:PRAYER_RULES.radianceThreshold,nether:PRAYER_RULES.netherThreshold};
export function normalizePrayerState(state){if(!state)return state;for(const [key,max]of Object.entries(prayerCaps))if(Number.isSafeInteger(state[key])&&state[key]>=0&&state[key]<=1e9)state[key]=Math.min(state[key],max);return state;}
export function validPrayerState(state){return !!state&&Object.entries(prayerCaps).every(([key,max])=>Number.isSafeInteger(state[key])&&state[key]>=0&&state[key]<=max)&&Array.isArray(state.mutations)&&state.mutations.every(key=>Object.hasOwn(PRAYER_MUTATIONS,key))&&new Set(state.mutations).size===state.mutations.length;}
...
export function resolvePrayer(person,skills,side,chances,lifeStages,random=Math.random){
 ...
 if(side==='front'){
  ...
  if(random()<chances.radianceChance/100&&person.prayer.radiance<PRAYER_RULES.radianceThreshold){person.prayer.radiance++;result.radiance=true;result.radianceTransformed=!wasRadiant&&isRadiant(person);}
 }else if(side==='back'){
  const wasNether=isNether(person);
  if(random()<chances.netherChance/100&&person.prayer.nether<PRAYER_RULES.netherThreshold){person.prayer.nether++;result.nether=true;result.transformed=!wasNether&&isNether(person);}
...
export function blessingMessage(result){
 ...
 if(result.transformed&&!result.converged)rewards.push('蜕变为幽冥族，一只眼睛觉醒为幽冥眼');
 if(result.converged)rewards.push('曦光与幽冥双重觉醒为两仪族，淡金曦轮与单侧幽冥眼共存');
