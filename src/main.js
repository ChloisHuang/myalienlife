...
import {Handshake,MessageCircleWarning,Waves} from 'lucide';
...
import {UFO_WEAR_PER_FLIGHT,backDiscovered,ufoDefinition,shipFoodStatus,fleetLimit,ufoInTransit} from './space-logistics.js';
...
import {studyFacilitySkill,FOUNDATION_SKILLS,FOUNDATION_FOR,MAJORS,higherEducation,studyName,educationRequirements,EDUCATION_LEVELS,educationLevel,educationWage,isLearner,studyInterest,canStudy,CONTINUING_EDUCATION_CAREER} from './education.js';
import {projectActions,workbench} from './settlements.js';
import {environmentPreferenceSummary,settlementIsland as settlementOf} from './island-preferences.js';
...
Object.assign(icons,{Handshake,MessageCircleWarning,Waves});
const hosted=import.meta.env.VITE_SERVER_AUTHORITY==='1';
const persistence=hosted?null:createPersistence();let game;
const presentation=hosted?createPresentation():null;let visualGame,visitorsUi,identityCard,dossierResidentUid=null;
...
},onStatus(value){visitorsUi?.setAuthenticated(value.authenticated);if(!identityCard)return;identityCard.setStatus(value);if(!value.canOperate&&build){build=false;cancelPlacement();document.querySelector('#build-button').classList.remove('active');}}}):null;
...
$('#app').innerHTML=`
 ...
 <main class="scene-ui">
  ...
  <div id="queue-wrap"><div class="queue-label"><span>行动队列</span><small>点击 × 取消</small><span id="queue-current" aria-live="polite"></span></div><div id="queue"></div><p id="autonomy-reason" hidden></p></div>
  <div class="camera-controls"><div class="island-tools"><span id="island-side"></span><button id="flip-island" aria-label="翻转星岛">${icon('Rotate3D')} 翻转星岛</button></div><div class="view-tools">${buttons([['拉近视角','Plus','id="zoom-in"'],['拉远视角','Minus','id="zoom-out"'],['进入全屏','Scan','id="fullscreen" aria-pressed="false"'],['跟随凯伊','LocateFixed','id="focus-player"'],['环境音乐','VolumeX','id="sound"']])}</div></div>
 ...
 <div id="loading"><div class="loading-orbit">${icon('Orbit')}</div><h2>正在降落露米纳星湾</h2><p>唤醒居民，点亮蘑菇，准备新的日常…</p><div class="boot-progress" role="progressbar" aria-label="资源加载进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><i></i></div><p class="boot-percent">已加载资源 0%</p></div>
...
if(hosted){
 identityCard=createIdentityCard({online,container:$('.location'),toast});visitorsUi=createVisitorsUi(online);online.start();
...
function showUfo(id){
 ...
 const ship=game.space.ships.find(s=>s.id===id);if(!ship)return;const def=ufoDefinition(ship),dialog=document.createElement('dialog'),dispatchTarget=islandOf(game.player),transit=ship.flight,dispatchIssue=transit?'正在飞行中':ship.reservedBy!==null?'已安排航行，请先取消登船安排':ship.durability<UFO_WEAR_PER_FLIGHT?'耐久不足':def.range<islandDefinition(game,dispatchTarget).level?'航程不足':null,status=transit?`${transit.kind==='return'?'自动返航':'调度'}中 · ${islandDefinition(game,transit.from.island).name} → ${islandDefinition(game,transit.to.island).name}`:`悬浮停靠于${islandDefinition(game,ship.island).name} · ${SIDES[ship.side]}`;dialog.className='ufo-dialog';
 dialog.innerHTML=`<button class="dialog-close" aria-label="关闭 UFO">${icon('X')}</button><h2>${def.name}</h2><p>${status}</p><p>${def.seats} 个座位 · 耐久 ${ship.durability}/100 · 可达科技 ${def.range} 级星岛 · 舱内食物 ${ship.food} / ${shipFoodStatus(ship).capacity} 份 · ${shipFoodStatus(ship).full?'补给满载':'补给未满'}</p><p>只有载客航段消耗 10 耐久，空船返航和远程调度不消耗耐久；载客抵达后，UFO 会留在抵达的星岛，等待下一次调度。耐久耗尽后到站回收。每位乘客每次载客航行消耗 1 份食物；当前星球有在世星厨时，缺粮禁止出航；无星厨时每缺 1 份食物，每位乘客扣 10 营养、5 能量。</p>${sameSide(ship,game.player)?'':`<button class="island-launch" data-dispatch-ufo="${ship.id}" ${dispatchIssue?'disabled':''} title="${dispatchIssue||''}">调度到${islandDefinition(game,dispatchTarget).name} · ${SIDES.front}</button>`}<button class="island-launch" data-load-ship="${ship.id}" ${transit?'disabled':''}>从岛上库存装载食物</button><p>${transit?'UFO 正在飞行，抵达后可继续安排。':ship.reservedBy?'已安排航行，正在等待乘客登船。':'选择目的地后安排乘客。'}</p>${Object.entries(islandCatalog(game)).filter(([destination])=>destination!==ship.island&&discovered(game,destination)).map(([destination,island])=>{const error=voyageError(game,game.player,game.skills,destination,1,null,ship.id);return `<button class="island-launch" data-voyage="${destination}" data-ship="${ship.id}" ${error?'disabled':''} title="${error||''}">前往${island.name}${error?` · ${error}`:''}</button>`;}).join('')}`;
 const cargo=game.space.cargo[id]??0,stock=game.space.materials[ship.island]??0,max=Math.min(stock,def.cargoCapacity-cargo),locked=ufoInTransit(ship)||ship.reservedBy!==null||!sameSide(ship,game.player);
...
function buildFleetPanel(){
 ...
 return `<section class="ufo-build-panel" aria-label="UFO 舰队"><div class="ufo-build-heading"><h3>${icon('Orbit')} UFO 舰队</h3><small>${ships.length} / ${fleetLimit(game)} 艘</small></div>${ships.length?`<div class="ufo-build-list">${ships.map(ship=>{const def=ufoDefinition(ship),reserved=ship.reservedBy!==null,transit=ship.flight,busy=reserved||!!transit,status=transit?`${transit.kind==='return'?'自动返航':'调度'}中 · ${islandDefinition(game,transit.from.island).name} → ${islandDefinition(game,transit.to.island).name}`:`${islandDefinition(game,ship.island).name} · ${SIDES[ship.side]} · ${reserved?'已安排':'空闲'}`;return `<div class="ufo-build-row"><div><strong>${def.name}</strong><small>${status} · 耐久 ${ship.durability}/100</small></div><button type="button" class="ufo-remove-button" data-remove-ufo="${ship.id}" ${busy?'disabled':''} aria-label="删除 UFO" title="${busy?(transit?'飞行中，请等待抵达':'已安排航行，请先取消登船安排'):'删除并回收 UFO'}">${icon('Trash2')}</button></div>`}).join('')}</div>`:'<p class="ufo-build-empty">暂无 UFO</p>'}</section>`;
}
...
function refreshDossier(){
 ...
 if(selectedResident==='player')$('#activity').innerHTML=q?`${icon(ACTIONS[q.type].icon)} ${currentActionLabel(q)}`:`${icon('Coffee')} 享受此刻的宁静`;
...
function renderConfig(){
 ...
 const educationRows=`<div class="config-row"><span>学习成功概率<small>education.studySuccessChance</small></span><label class="config-number">概率 ${configInput('education.studySuccessChance',config.education.studySuccessChance,0,100,.1)} %</label><em>每次学习完成时独立判定，成功才获得学分和经验</em></div>`;
...
function renderPanel(){
 ...
 if(tab==='career'&&!learning){
  ...
  const careerSummary=studying?'无收入 · 专心提升学历':`${educationWage(person,level.wage)} 星币 / 班次 · ${educationLevel(person).name} ×${displayNumber(educationLevel(person).multiplier)} <span>·</span> ${next?`晋升要求：${state.shifts} / ${next.shifts} 班次 · ${careerSkillsText(skills,state.id,state.level+1)}`:'已达最高等级'}`;
...
function refreshLivingMenu(){
 if(!context)return;
 const other=context.kind==='npc'?game.npcs[context.id]:null,o=context.kind==='object'?game.objects.find(o=>o.id===context.id):null;
 let status=$('#living-status');
 if(!other&&!o)return;
 if(!status){status=document.createElement('p');status.id='living-status';$('#context-menu .context-heading').after(status);}
 if(other){status.textContent=[relationLabel(game,game.player,other),`对方：${relationLabel(game,other,game.player)}`,livingSummary(game,other)].filter(Boolean).join(' · ');}
 else if(o.type==='spiritTree'&&supportedIsland(o))status.textContent=`圣树活力 ${livingSite(game,o).vitality} / 100`;
 else if(o.plant&&islandOf(o)==='spore'){const keeper=[game.player,...neighbors(game)].find(p=>p.uid===livingSite(game,o).keeper);status.textContent=keeper?`花圃记得${keeper.name}的照料`:'尚未形成共生联系';}
 else if(livingOptions(game,o).includes('listenForest')){const s=livingResident(game,game.player);status.textContent=`影子适应 ${s.shadow} / 100 · 不安 ${s.fear} / 100`;}
 else status.textContent='';
 for(const b of document.querySelectorAll('#context-menu [data-action]'))if(Object.hasOwn(LIVING_ACTIONS,b.dataset.action)||b.dataset.action==='pray'){
  const error=livingError(game,b.dataset.action,game.player,o,other);b.disabled=!!error;b.title=error||'';
 }
}
function refresh(){
 ...
 if(context?.kind==='npc'&&$('#npc-status')){const n=game.npcs[context.id];$('#npc-status').textContent=`${n.activity} · ${n.ai.reason}`;}
  const current=game.queue[0];$('#queue-current').innerHTML=current?`${icon(ACTIONS[current.type].icon)} ${currentActionLabel(current)}`:'';
...
function refreshProjectMenu(){
 ...
 if(!workbench(o)||!project&&islandOf(o)!=='eva')return;
 if(project){let status=$('#project-status');if(!status){status=document.createElement('p');status.id='project-status';$('#context-menu .context-heading').after(status);}
...
async function showContext(target,x,y){
 ...
 refreshProjectMenu();refreshProfessionMenu();refreshLivingMenu();el.style.maxHeight=`${innerHeight-105}px`;el.style.top=`${Math.max(85,Math.min(y-40,innerHeight-el.offsetHeight-16))}px`;
...
$('#app').addEventListener('click',async e=>{
 ...
 if(b.dataset.career){const id=b.dataset.career;if(await setCareer(game,id)){toast(`已加入${CAREERS[id].name}职业。`);lastPanel='';refresh();}else toast(careerEntryMessage(game,id)||'请先完成或取消当前工作或学习安排。');}
...
try{
 ...
 const bootBar=$('#loading .boot-progress'),bootFill=$('#loading .boot-progress i'),bootPercent=$('#loading .boot-percent');
 const showBootProgress=ratio=>{const clamped=Math.max(0,Math.min(1,ratio)),percent=Math.round(clamped*100);bootFill.style.transform=`scaleX(${clamped})`;bootBar.setAttribute('aria-valuenow',String(percent));bootPercent.textContent=translateText(`已加载资源 ${percent}%`);};
 showBootProgress(0);
 world=await createWorld($('#world'),()=>game,{
  onBootProgress:showBootProgress,
  onClick:showContext,
