...
import {NEEDS,neighbors,birthDecision,gameMinutes,takeOver,CAREERS,missingCareerSkills,careerEntryMessage,ITEMS,ACTIONS,createGame,tick,enqueue,cancelAction,buyItem,sellItem,setCareer,setAutonomy,sellHarvest,updateResident,canAffordAction} from './simulation.js';
...
function careerSkillsText(skills,id,level=1){const requirements=CAREERS[id]?.levels[level-1]?.skills||{};return Object.entries(requirements).map(([key,required])=>`${SKILLS[key].name} ${Math.floor(skills[key]??0)}/${required}`).join(' · ')||'无';}
function careerMissingText(skills,id,level=1){return missingCareerSkills(skills,id,level).map(({key,current,required})=>`${SKILLS[key].name} ${Math.floor(current)}/${required}`).join('、');}
