...
import {NEEDS,neighbors,birthDecision,gameMinutes,takeOver,CAREERS,ITEMS,ACTIONS,createGame,tick,enqueue,cancelAction,buyItem,sellItem,setCareer,setAutonomy,sellHarvest,updateResident} from './simulation.js';
...
$('#app').innerHTML=`
 ...
 <dialog id="harvest-dialog"><button id="close-harvest" aria-label="关闭收成仓库">${icon('X')}</button><span class="eyebrow">星湾的收获</span><h2>收成仓库</h2><p>居民采收的产物会存放在这里，由你决定何时出售。</p><div id="harvest-content"></div></dialog><div id="tooltip" hidden></div><div id="context-menu" hidden></div><div id="toast" role="status" hidden></div>
