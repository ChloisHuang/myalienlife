...
function refreshPortraits(){
 if(!world)return;const activeId=activeResidentId(),entries=[{id:activeId,person:game.player},...neighbors(game).map(n=>({id:n.id,person:game.npcs[n.id]}))],key=`${activeId}|${JSON.stringify(game.config.lifeStages)}|${entries.map(({id,person})=>`${id}-${person.uid}-${person.name}-${person.alive}-${person.gender}-${lifeStage(person.age,game.config.lifeStages)}-${JSON.stringify(person.genome)}`).join('|')}`;if(key===portraitKey)return;portraitKey=key;
...
function showContext(target,x,y){
 ...
 else{const obj=game.objects.find(o=>o.id===target.id);const item=ITEMS.find(i=>i.id===obj.type);title=item.name;subtitle=item.desc;actions=[item.action];if(CROPS[obj.type])actions=['garden','harvest','replant'];if(obj.type===workStationType(game.career.id))actions.push('work');if(obj.type==='sofa')actions.push('lounge');if(obj.type==='nursery'){const birth=game.incubations.find(b=>b.podId===obj.id);if(birth){subtitle=`${birth.parents.map(p=>p.name).join('与')}的星芽 · 还有 ${Math.ceil(birth.due-gameMinutes(game))} 游戏分钟出生`;actions=[];}}}
