import {UFO_WEAR_PER_FLIGHT,backDiscovered,ufoDefinition,shipFoodStatus,loadShipFood} from './space-logistics.js';
...
function renderPanel(){
 ...
 if(signature===lastPanel||tab==='life'&&document.activeElement?.matches('#panel-content select'))return;lastPanel=signature;
  $('#panel-tag').textContent=tab==='exploration'?'文明、记录与共享资源':tab==='needs'?'六种需求 · 一种好生活':tab==='relations'?'友谊也需要悉心照料':tab==='career'?'在银河找到自己的位置':tab==='skills'?'通过实际行动积累经验':tab==='resident'?`每 ${game.config.time.starYearDays} 天增长 1 星岁`:tab==='life'?'新生、陪伴与告别':build?'选择物品 → 点击地面摆放':`3 个主题包 · ${ITEMS.length} 件物品`;
...
function refresh(){
 ...
 refreshWonderMenu();refreshProjectMenu();
...
function refreshProjectMenu(){
 ...
 if(!project||!['lab','stove'].includes(o.type))return;
 ...
 status.textContent=`蓝图 ${Math.floor(project.blueprint/300*100)}% · 建设 ${Math.floor(project.construction/600*100)}%`;
 for(const b of document.querySelectorAll('#context-menu [data-action]'))if(['developBlueprint','constructIsland','settleIsland'].includes(b.dataset.action)){const error=civilizationError(game,b.dataset.action,o,game.player,game.skills);b.disabled=!!error;b.title=error||'';}
}
...
function showContext(target,x,y){
 ...
 else{const obj=game.objects.find(o=>o.id===target.id);const item=ITEMS.find(i=>i.id===obj.type);title=item.name;subtitle=item.desc;if(obj.type==='spiritTree')subtitle=sideOf(obj)==='front'?`跪下祈祷 · 独立 ${game.config.prayer.radianceChance}% 概率曦光属性 +1，${PRAYER_RULES.radianceThreshold} 点觉醒曦灵族 · ${game.config.prayer.skillChance}% 概率随机未满级技能升 1 级 · 长者独立 ${game.config.prayer.rejuvenationChance}% 概率返老还童至 ${game.config.lifeStages.infantEnd} 星岁`:`跪下祈祷 · ${game.config.prayer.netherChance}% 概率幽冥属性 +1，${PRAYER_RULES.netherThreshold} 点蜕变幽冥族 · 独立 ${game.config.prayer.mutationChance}% 概率永久变异，可多种共存`;actions=[...(WONDER_OPTIONS[obj.type]||[item.action])];if(obj.type==='portal')actions.push('memoryExpedition');if(obj.type==='lab')actions.push('spaceResearch','buildUfo1','buildUfo2','buildUfo3');if(obj.type==='stove')actions.push('prepareRations');if(['lab','stove'].includes(obj.type)&&islandOf(obj)!=='home')actions.push('developBlueprint','constructIsland','settleIsland');if(CROPS[obj.type])actions=['garden','harvest','replant'];if(obj.type===workStationType(game.career.id))actions.push('work');if(obj.type==='sofa')actions.push('lounge');if(obj.type==='nursery'){const birth=game.incubations.find(b=>b.podId===obj.id);if(birth){subtitle=`${birth.parents.map(p=>p.name).join('与')}的星芽 · 还有 ${Math.ceil(birth.due-gameMinutes(game))} 游戏分钟出生`;actions=[];}}}
 ...
 el.style.maxHeight=`${innerHeight-105}px`;el.style.top=`${Math.max(85,Math.min(y-40,innerHeight-el.offsetHeight-16))}px`;
