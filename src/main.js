...
function refreshPortraits(){
 if(!world)return;const key=`${JSON.stringify(game.config.lifeStages)}|${[game.player,...Object.values(game.npcs)].map(p=>`${p.uid}-${p.alive}-${p.gender}-${lifeStage(p.age,game.config.lifeStages)}`).join('|')}`;if(key===portraitKey)return;portraitKey=key;
...
function refresh(){
 ...
 $('#life-alert').hidden=game.player.alive&&!endangered.length;$('#life-alert').textContent=!game.player.alive?`${game.player.name}已离世。时间已暂停，请在生命页选择接管居民。`:`${endangered.map(n=>n.name).join('、')}持续饥饿，请立即进食或安排照料。`;
  const q=game.queue[0];$('#activity').innerHTML=q?`${icon(ACTIONS[q.type].icon)} ${q.phase==='walking'?'正在走过去…':q.phase==='waiting'?'等待家具空闲…':actionLabel(q.type)}`:`${icon('Coffee')} 享受此刻的宁静`;
 ...
 if(context?.kind==='npc'&&$('#npc-status')){const n=game.npcs[context.id];$('#npc-status').textContent=`${n.activity} · ${n.ai.reason}`;}
  $('#queue').innerHTML=game.queue.length?game.queue.map((q,i)=>`<div class="queue-action ${i===0?'current':''}" style="--progress:${q.phase==='acting'?Math.min(100,q.elapsed/Math.max(.1,game.config.actionDurations[q.type])*100):0}%">${icon(ACTIONS[q.type].icon)}<span>${actionLabel(q.type)}</span>${q.source==='ai'?'<small class="ai-badge">自主</small>':''}<button data-cancel="${q.id}" aria-label="取消${actionLabel(q.type)}">${icon('X')}</button></div>`).join(''):`<span class="queue-empty">${icon('MousePointer2')} ${game.autonomy.enabled?'正在考虑下一件小事':'给今天安排一点小事吧'}</span>`;
...
$('#app').addEventListener('click',e=>{
 ...
 if(b.dataset.action&&context){const result=enqueue(game,b.dataset.action,context.id,undefined,$('#birth-partner')?.value||null);if(!result.ok)toast(result.message);else toast(`已安排：${actionLabel(b.dataset.action)}`);closeContext();refresh();}
