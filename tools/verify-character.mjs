...
try{
 const page=await browser.newPage({viewport:{width:1200,height:800}}),errors=[];page.on('console',m=>{if(m.type()==='error')console.log(m.text());});page.on('pageerror',e=>errors.push(e.message));
 ...
 const source=(await new GLTFLoader().loadAsync('/assets/alien.glb')).scene;stylizeAsset(source,{soft:true});
 ...
 await writeFile('artifacts/character-walk-closeup.webm',Buffer.from(video,'base64'));if(errors.length)throw new Error(errors.join('\n'));console.log('CHARACTER_CLOSEUP_VERIFIED');
