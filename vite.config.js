...
import {remoteSaveApi} from './server/remote-save-api.js';
...
const mount=server=>{server.middlewares.use(projectConfigApi(projectConfig));server.middlewares.use(remoteSaveApi(store));server.middlewares.use(saveApi(store));};
