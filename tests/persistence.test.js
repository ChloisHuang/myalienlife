...
test('local save store can atomically replace the current world for an explicit import',async t=>{const {store}=await setup(t),local=createGame(),remote=createGame();local.day=3;remote.day=159;await store.write(request(local));assert.equal(typeof store.replace,'function');const saved=await store.replace(remote,{clientId:'remote-import-test'});assert.equal(saved.revision,2);assert.equal(saved.state.day,159);assert.equal((await store.read()).state.day,159);});
test('development remote-save endpoint copies remote state into the local save without remote writes',async t=>{
 assert.equal(typeof remoteImplementation.remoteSaveApi,'function');const {store}=await setup(t),local=createGame(),remote=createGame();local.day=4;remote.day=207;await store.write(request(local));const calls=[];
 const middleware=remoteImplementation.remoteSaveApi(store,{remoteOrigin:'https://remote.example:6443',fetchImpl:async(url,options)=>{calls.push([url,options]);return{ok:true,status:200,json:async()=>({revision:91,state:remote})};}}),server=createServer((req,res)=>middleware(req,res,()=>{res.statusCode=404;res.end();}));
 await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));t.after(()=>new Promise(resolve=>server.close(resolve)));const response=await fetch(`http://127.0.0.1:${server.address().port}/api/pull-remote-save`,{method:'POST'}),body=await response.json();assert.equal(response.status,200);assert.equal(body.ok,true);assert.equal((await store.read()).state.day,207);assert.equal(calls.length,1);assert.equal(calls[0][0],'https://remote.example:6443/api/state');assert.equal(calls[0][1].method,'GET');
});
test('development remote-save endpoint rejects cross-origin overwrite attempts',async t=>{
 const {store}=await setup(t),local=createGame(),remote=createGame();local.day=8;remote.day=99;await store.write(request(local));let remoteReads=0;const middleware=remoteImplementation.remoteSaveApi(store,{remoteOrigin:'https://remote.example:6443',fetchImpl:async()=>{remoteReads++;return{ok:true,status:200,json:async()=>({state:remote})};}}),server=createServer((req,res)=>middleware(req,res,()=>{res.statusCode=404;res.end();}));await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));t.after(()=>new Promise(resolve=>server.close(resolve)));
 const response=await fetch(`http://127.0.0.1:${server.address().port}/api/pull-remote-save`,{method:'POST',headers:{Origin:'https://evil.example'}});assert.equal(response.status,403);assert.equal(remoteReads,0);assert.equal((await store.read()).state.day,8);
});
