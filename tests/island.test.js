...
test('both faces and cooking survive save roundtrip; version 8 upgrades explicitly',()=>{
 ...
 const upgraded=restore(serialize(old));assert.equal(upgraded.version,21);assert.equal(upgraded.skills.cooking,0);assert.equal(upgraded.player.side,'front');assert.equal(upgraded.objects.filter(o=>o.type==='gate').length,2);
...
test('area lamps can be purchased on either face, used, saved and sold',()=>{
 ...
 result.object.wonder.bugs=1;assert.equal(enqueue(g,'catchBugs',result.object.id).ok,true);run(g,45);assert.equal(g.player.side,'back');
