...
test('both faces and cooking survive save roundtrip; version 8 upgrades explicitly',()=>{
 ...
 const upgraded=restore(serialize(old));assert.equal(upgraded.version,23);assert.equal(upgraded.skills.cooking,0);assert.equal(upgraded.player.side,'front');assert.equal(upgraded.objects.filter(o=>o.type==='gate').length,2);
