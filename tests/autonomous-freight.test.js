...
function setup(tier=2){
 ...
 g.civilization.discoveryPath=['eva','spore'];g.civilization.technology=240;
 g.space.materials.eva=80;g.space.ships=[{id:'freighter',tier,island:'eva',side:'front',food:10,durability:100,reservedBy:null}];return g;
}
...
test('autonomous flight loads construction materials, delivers them, and an architect builds without manual loading',()=>{
 ...
 until(g,()=>g.queue[0]?.phase==='acting');assert.equal(g.space.cargo.freighter,60);assert.equal(g.space.materials.eva,20);
 ...
 // The architect settles here: a visitor would keep wanting the trip home (and can now call a ship for it), which is a different scenario.
 loaded.player.settlementIsland='spore';
...
test('automatic loading respects capacity and leaves material transactions conserved',()=>{
 const g=setup(1);arrange(g);until(g,()=>g.queue[0]?.phase==='acting');assert.equal(g.space.cargo.freighter,20);assert.equal(g.space.materials.eva,60);
 cancelAction(g,g.queue[0].id);assert.equal(g.space.cargo.freighter,20);arrange(g);until(g,()=>g.queue[0]?.phase==='acting');assert.equal(g.space.cargo.freighter,20);assert.equal(g.space.materials.eva,60);
});
...
test('only the unpaid construction deficit is shipped, accounting for destination stock and existing cargo',()=>{
 ...
 assert.equal(g.space.cargo.freighter,5);assert.equal(g.space.materials.eva,77);
});

test('manual flights and completed destinations do not automatically take materials',()=>{
 for(const completed of [false,true]){const g=setup();if(completed){g.civilization.projects.spore.blueprint=300;g.civilization.projects.spore.construction=600;}arrange(g,completed);until(g,()=>g.queue[0]?.phase==='acting');assert.equal(g.space.cargo.freighter,undefined);assert.equal(g.space.materials.eva,80);}
});

test('canceling before departure never takes building materials from storage',()=>{
 const g=setup();arrange(g);cancelAction(g,g.queue[0].id);assert.equal(g.space.cargo.freighter,undefined);assert.equal(g.space.materials.eva,80);
});
...
test('an NPC pilot can load local materials while the controlled resident is elsewhere',()=>{
 ...
 until(g,()=>g.npcs.kai.queue[0]?.phase==='acting');assert.equal(g.space.cargo.freighter,60);assert.equal(g.space.materials.eva,20);
});
...
test('other reserved inbound cargo prevents duplicate supply, and canceled shipments release that demand',()=>{
 ...
 g.space.ships.push({id:'inbound',tier:2,island:'eva',side:'front',food:4,durability:100,reservedBy:999});
 ...
 assert.equal(loadConstructionCargo(g,q),50);assert.equal(g.space.cargo.freighter,60);assert.equal(g.space.materials.eva,20);
});
