...
test('v23 legacy city is evacuated like a destroyed island while global progress is preserved',()=>{
 const g=createGame();
 g.version=23;
 g.civilization.discoveryPath=['eva','spore','city'];
 ...
 g.player.island='city';g.player.settlementIsland='city';g.player.preferences.memoryExpedition=90;
 g.npcs.nova.island='city';g.npcs.nova.settlementIsland='city';g.npcs.nova.preferences.memoryExpedition=60;
 ...
 g.space.provisions.eva=5;g.space.provisions.city=8;
 g.space.materials.eva=6;g.space.materials.city=11;g.space.backs.city=true;
 ...
 assert.equal(loaded.player.island,'eva');assert.equal(loaded.player.settlementIsland,'eva');
 assert.equal(loaded.npcs.nova.island,'eva');assert.equal(loaded.npcs.nova.settlementIsland,'eva');
 assert.equal(loaded.viewIsland,'eva');assert.equal(loaded.viewSide,'front');
 ...
 assert.equal(loaded.objects.find(o=>o.id==='legacy-city-nursery').island,'eva');
 assert.equal(loaded.incubations.some(b=>b.id==='legacy-starbud'),true);
 assert.equal(loaded.space.ships.find(s=>s.id==='legacy-city-ship').island,'eva');
 assert.equal(loaded.space.ships.find(s=>s.id==='legacy-city-ship').side,'front');
 assert.equal(loaded.space.provisions.eva,13);assert.equal(loaded.space.provisions.city,undefined);
 assert.equal(loaded.space.materials.eva,17);assert.equal(loaded.space.materials.city,undefined);assert.equal(loaded.space.backs.city,undefined);
