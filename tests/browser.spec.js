...
test('loading day 159 with generated islands initializes dropdown arrows without resetting the displayed day',async({page})=>{
 const {discoverProceduralIslands}=await import('../src/civilization.js');const state=createGame();state.day=159;state.speed=0;state.civilization.observations=18;state.wonders.archive=3;discoverProceduralIslands(state.civilization);fixtures.set(page,state);
...
test('procedural island renders its saved blueprint after physical landing and reloading',async({page})=>{
 const {discoverProceduralIslands}=await import('../src/civilization.js');const state=createGame();state.speed=0;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;state.civilization.seed=20260908;state.civilization.observations=12;state.civilization.technology=240;state.skills.science=18;state.space.ships.push({id:'fixture-ufo',tier:3,island:'home',side:'front',food:2,reservedBy:null});discoverProceduralIslands(state.civilization);state.skills.botany=18;state.player.preferences.garden=10;fixtures.set(page,state);
...
test('qualified resident lands on a distinct island, views home without teleporting, reloads and returns',async({page})=>{
 ...
 state.civilization.observations=3;state.civilization.technology=120;state.space.ships.push({id:'fixture-ufo',tier:2,island:'home',side:'front',food:2,reservedBy:null});state.skills.botany=6;state.skills.science=12;state.player.preferences.observe=10;fixtures.set(page,state);
...
test('UFO passenger picker carries all races after earlier work and remote back can be viewed without moving residents',async({page})=>{
 const {enqueue,tick,switchControl}=await import('../src/simulation.js');const state=createGame();state.civilization.observations=3;state.civilization.technology=120;state.skills.science=6;state.space.ships.push({id:'family-ufo',tier:2,island:'home',side:'front',food:0,reservedBy:null});state.space.provisions.home=8;state.npcs.nova.prayer.nether=10;state.npcs.lumi.prayer.radiance=10;state.npcs.pip.age=1;
...
test('UFO food indicators distinguish partial and full cargo and loading uses stored food',async({page})=>{
 const state=createGame();state.speed=0;state.minute=1100;state.space.ships=[{id:'supply-ship',tier:3,island:'home',side:'front',food:4,reservedBy:null}];state.space.provisions.home=20;fixtures.set(page,state);const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-ufo]').click();await expect(page.locator('.ufo-dialog')).toContainText('4 / 24 份 · 补给未满');await page.getByRole('button',{name:'关闭 UFO',exact:true}).click();await page.screenshot({path:'artifacts/ufo-food-partial.png'});
...
test('UFO flight displays a persistent route and animated departure before arrival',async({page})=>{
 const {enqueue}=await import('../src/simulation.js');const state=createGame();state.civilization.technology=120;state.civilization.observations=3;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
 state.space.ships=[{id:'animated-ufo',tier:2,island:'home',side:'front',food:8,reservedBy:null}];expect(enqueue(state,'voyage','portal',undefined,null,'spore',[],'animated-ufo').ok).toBe(true);
