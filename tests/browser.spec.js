...
test('island switching frame trace',async({page},testInfo)=>{
 const g=createGame();g.speed=0;g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;fixtures.set(page,g);
 ...
 for(const id of ['spore','eva','spore','eva']){await page.locator('#island-select').selectOption(id);await page.waitForTimeout(800);}
...
test('island previews are local cached scene captures on desktop and mobile',async({page})=>{
 const g=createGame();g.speed=0;g.viewIsland='spore';g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;g.civilization.projects.spore.blueprint=300;g.civilization.projects.spore.construction=600;fixtures.set(page,g);
 ...
 const settlement=page.locator('[data-thumb="eva"] canvas');await expect(settlement).toHaveCount(1);
 ...
 await page.locator('#island-select').selectOption('eva');await page.locator('#island-select').selectOption('spore');await expect(page.locator('[data-thumb="spore"] canvas')).toHaveCount(1);
...
test('fixed preview camera preserves the main framebuffer and reuses unchanged captures',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const game={viewIsland:'eva',viewSide:'front',civilization:{projects:{}},objects:[]};
  await previews.update(game,0);const canvas=previews.get('eva'),first=canvas.toDataURL(),frame=renderer.info.render.frame;
  ...
  game.objects.push({id:'new',type:'chair',island:'eva',side:'front',x:0,z:0,rotation:0});
  await previews.update(game,2000);const fixed=previews.get('eva').toDataURL()===first;
  ...
  game.viewIsland='spore';await previews.update(game,4000);game.viewIsland='eva';await previews.update(game,5000);
  const revisitFrozen=canvas.toDataURL()===first;
  await previews.update(game,300001);const updated=previews.get('eva')===canvas&&canvas.toDataURL()!==first;
...
test('resident dossiers slide closed and switch control',async({page})=>{
 ...
 await expect(page.locator('#world')).toHaveAttribute('data-island','eva');
...
test('phone dossiers start closed and portrait renders a usable horizontal scene',async({page})=>{
 ...
 const g=createGame();g.speed=0;g.autonomy.enabled=false;g.objects=[];g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;fixtures.set(page,g);
 await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('mushroom variants render on desktop and mobile; harvesting clears the plant and menu',async({page})=>{
 ...
 state.objects=[{}, {giant:true}, {cluster:true}, {mutant:true}, {mutant:true,cluster:true}].map((traits,i)=>({id:`crop-${i}`,type:'mushroom',x:-8+i*4,z:2,rotation:0,side:'front',island:'eva',plant:{...createPlant(),growth:1,...traits}}));
...
test('build mode exposes selling for fixed default furniture',async({page})=>{
 const state=createGame();state.speed=0;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;state.objects.push({id:'fixed-default-sofa',type:'sofa',x:0,z:4,rotation:0,island:'eva',side:'front',fixed:true});fixtures.set(page,state);
...
test('hovering cargo shelves and UFOs shows local stock and loaded capacity without opening a dialog',async({page})=>{
 ...
 state.objects=[{id:'cargo-shelf',type:'materialCabinet',x:0,z:3,island:'eva',side:'front',rotation:0},{id:'cargo-platform',type:'loadingPlatform',x:5,z:3,island:'eva',side:'front',rotation:0}];
 state.space.materials.eva=42;state.space.ships=[{id:'hover-ship',tier:2,island:'eva',side:'front',food:3,durability:80,reservedBy:null},{id:'back-ship',tier:1,island:'eva',side:'back',food:0,durability:100,reservedBy:null}];state.space.cargo={'hover-ship':12,'back-ship':7};fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('cargo hover inventory refreshes while the pointer stays still',async({page})=>{
 ...
 state.objects=[plant,{id:'live-shelf',type:'materialCabinet',x:0,z:3,island:'eva',side:'front',rotation:0}];state.career.id='botanist';plant.plant.growth=1;state.player.x=plant.x;state.player.z=plant.z+1;
...
test('Atoll preview follows the server save without starting or saving a game',async({page})=>{
 const state=createGame();state.day=159;state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;state.viewIsland=state.player.island='spore';fixtures.set(page,state);
 ...
 state.day=160;state.viewIsland='eva';fixtures.set(page,state);await expect(page.locator('#world')).toHaveAttribute('data-day','160');await expect(page.locator('#world')).toHaveAttribute('data-island','eva');
...
test('left outer UFO fleet renders on desktop and mobile without blocking the rooms',async({page})=>{
 const state=createGame();state.speed=0;state.space.ships=[1,2,3].map(tier=>({id:`left-fleet-${tier}`,tier,island:'eva',side:'front',food:4,durability:100,reservedBy:null}));fixtures.set(page,state);
...
test('professional packs render six devices and expose construction, extraction and shared cargo controls',async({page})=>{
 ...
 const state=createGame();state.speed=0;state.autonomy.enabled=false;state.career.id='architect';state.player.x=-8;state.player.z=5;state.space.materials.eva=24;
 const types=['blueprintTable','constructionTerminal','cultivator','extractor','materialCabinet','loadingPlatform'];state.objects=types.map((type,i)=>({id:type,type,x:(i%3-1)*5,z:i<3?-3:3,island:'eva',side:'front',rotation:0,...(type==='cultivator'?{plant:{growth:1,water:75,health:100,harvests:0,giant:false}}:{})}));
 state.space.ships.push({id:'pack-ship',tier:1,island:'eva',side:'front',food:4,durability:100,reservedBy:null});fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('restart epoch requires confirmation and persists a clean first day',async({page})=>{
 const state=createGame();state.day=40;state.money=50000;state.speed=0;state.space.materials.eva=80;state.civilization.discoveryPath=['eva','spore'];fixtures.set(page,state);
 ...
 await expect.poll(async()=> (await savedState(page))?.day).toBe(1);const saved=await savedState(page);expect(saved.civilization.discoveryPath).toEqual(['eva']);expect(saved.money).toBe(2400);expect(saved.space.materials).toEqual({});expect(saved.space.ships).toEqual([]);
...
test('cargo dialog loads and unloads local materials within ship capacity',async({page})=>{
 const state=createGame();state.speed=0;state.space.materials.eva=30;state.space.ships.push({id:'cargo-ui',tier:1,island:'eva',side:'front',food:4,durability:100,reservedBy:null});fixtures.set(page,state);
...
test('resident profile shows each residents settled island rather than the island being visited',async({page})=>{
 const state=createGame();state.speed=0;state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;state.player.island=state.viewIsland='spore';state.player.settlementIsland='eva';state.npcs.nova.settlementIsland='spore';fixtures.set(page,state);
 ...
 const settlement=page.locator('#resident-settlement-island');await expect(settlement).toHaveText('露米纳星湾');await page.screenshot({path:'artifacts/resident-settlement-desktop.png'});
 await page.locator('#active-character').click();await page.locator('[data-character="nova"]').click();await expect(settlement).toHaveText('童梦星屿');
 await page.locator('#active-character').click();await page.locator('[data-character="zig"]').click();await expect(settlement).toHaveText('露米纳星湾');
 await page.setViewportSize({width:390,height:844});await expect(settlement).toBeVisible();await page.screenshot({path:'artifacts/resident-settlement-mobile.png'});expect(errors).toEqual([]);
});

test('destroy island confirms, evacuates, preserves the next island and survives reload',async({page})=>{
 const state=createGame();state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;addGenerated(state,0,1);state.player.island=state.viewIsland='spore';state.player.settlementIsland='spore';state.speed=0;fixtures.set(page,state);
 ...
 const destroy=page.locator('[data-destroy-island="spore"]');await expect(destroy).toBeVisible();await expect(page.locator('[data-destroy-island="eva"]')).toHaveCount(0);
 ...
 await expect.poll(async()=> (await savedState(page))?.civilization.destroyedIslands).toEqual(['spore']);const saved=await savedState(page);expect(saved.player.island).toBe('eva');expect(saved.player.settlementIsland).toBe('eva');
...
test('settlement workbench gates construction behind blueprints and persists construction work',async({page})=>{
 const state=createGame();state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;state.player.island=state.viewIsland='spore';state.player.x=-2;state.player.z=1.2;state.speed=0;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
...
test('home construction terminal exposes the same migration action',async({page})=>{
 const state=createGame();state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;state.player.settlementIsland='spore';state.player.x=0;state.player.z=2;state.speed=0;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;state.objects.push({id:'home-settlement-terminal',type:'constructionTerminal',x:3,z:0,island:'eva',side:'front',rotation:0});fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('loading day 159 renders the portrait island selector without resetting the displayed day',async({page})=>{
 const state=createGame();state.day=159;state.speed=0;state.civilization.observations=18;state.wonders.archive=3;state.civilization.discoveryPath=['eva','spore'];addGenerated(state,0);addGenerated(state,1);fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('mobile island selector keeps a constant bar height through open and collapse motion',async({page})=>{
 await page.setViewportSize({width:844,height:390});
 const state=createGame();state.speed=0;state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;fixtures.set(page,state);
 await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
...
test('star island navigation shows discovery without granting landing or legacy location shortcuts',async({page})=>{
 const state=createGame();state.speed=0;state.civilization.observations=3;state.civilization.discoveryPath=['eva','spore'];state.wonders.archive=3;fixtures.set(page,state);
 await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});
 ...
 await page.locator('#island-select').selectOption('spore');await expect(page.locator('#world')).toHaveAttribute('data-island','eva');
...
test('qualified resident lands on a distinct island, views home without teleporting, reloads and returns',async({page})=>{
 ...
 await page.screenshot({path:'artifacts/star-island-spore.png'});await page.locator('#island-select').selectOption('eva');await page.locator('#save').click();let saved=await savedState(page);expect(saved.player.island).toBe('spore');expect(saved.viewIsland).toBe('eva');
 await page.reload();await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.locator('#island-select').selectOption('spore');await expect(page.locator('#world')).toHaveAttribute('data-island','spore');await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-voyage="eva"]').click();await page.locator('#confirm-flight').click();await page.locator('[data-speed="3"]').click();await expect(page.locator('#world')).toHaveAttribute('data-island','eva',{timeout:30000});await expect(page.locator('.ufo-flight-board')).toBeHidden({timeout:20000});await page.locator('[data-speed="0"]').click();await page.locator('#save').click();saved=await savedState(page);expect(saved.player.island).toBe('eva');expect(saved.civilization.visits.spore).toBe(1);expect(errors).toEqual([]);
});
...
test('quantum science master can return through portal without a UFO',async({page})=>{
 ...
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-star-voyage="eva"]').click();await page.locator('[data-speed="3"]').click();await expect(page.locator('#world')).toHaveAttribute('data-island','eva',{timeout:25000});await expect(page.locator('.ufo-flight-board')).toBeHidden({timeout:20000});await page.locator('[data-speed="0"]').click();await page.locator('#save').click();expect((await savedState(page)).space.ships).toHaveLength(0);expect(errors).toEqual([]);
});
test('UFO food indicators distinguish partial and full cargo and loading uses stored food',async({page})=>{
 const state=createGame();state.speed=0;state.minute=1100;state.space.ships=[{id:'supply-ship',tier:3,island:'eva',side:'front',food:4,durability:100,reservedBy:null}];state.space.provisions.eva=20;fixtures.set(page,state);const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-ufo]').click();await expect(page.locator('.ufo-dialog')).toContainText('4 / 24 份 · 补给未满');await page.getByRole('button',{name:'关闭 UFO',exact:true}).click();await page.screenshot({path:'artifacts/ufo-food-partial.png'});
 await page.locator('[data-ufo]').click();await page.locator('[data-load-ship]').click();await expect(page.locator('.ufo-dialog')).toContainText('24 / 24 份 · 补给满载');await page.getByRole('button',{name:'关闭 UFO',exact:true}).click();await page.screenshot({path:'artifacts/ufo-food-full.png'});await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships[0].food).toBe(24);expect(saved.space.provisions.eva).toBe(0);expect(errors).toEqual([]);
});
test('build mode removes an idle UFO and recovers its cargo and rations',async({page})=>{
 const state=createGame();state.speed=0;state.space.ships=[{id:'removable-ufo',tier:2,island:'eva',side:'front',food:4,durability:60,reservedBy:null}];state.space.cargo['removable-ufo']=12;state.space.materials.eva=8;state.space.provisions.eva=3;state.money=100;fixtures.set(page,state);const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-ufo="removable-ufo"]').click();await expect(page.locator('.ufo-dialog [data-remove-ufo]')).toHaveCount(0);await page.locator('.ufo-dialog .dialog-close').click();await page.locator('#build-button').click();await expect(page.locator('.ufo-build-panel')).toBeVisible();await expect(page.locator('[data-remove-ufo]')).toBeEnabled();page.once('dialog',dialog=>{expect(dialog.message()).toContain('删除');dialog.accept();});await page.locator('[data-remove-ufo]').click();await expect(page.locator('.ufo-build-panel')).toContainText('暂无 UFO');await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships).toHaveLength(0);expect(saved.space.cargo['removable-ufo']).toBeUndefined();expect(saved.space.materials.eva).toBe(20);expect(saved.space.provisions.eva).toBe(7);expect(saved.money).toBe(520);expect(errors).toEqual([]);
});
...
test('UFO moving approach is rendered on desktop and mobile',async({page})=>{
 const {enqueue}=await import('../src/simulation.js');const state=createGame();state.civilization.technology=120;state.civilization.observations=3;state.civilization.discoveryPath=['eva','spore'];state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
...
test('remote UFO can be dispatched from its dialog without spending a flight use',async({page})=>{
 const state=createGame();state.speed=0;state.civilization.technology=120;state.civilization.observations=3;state.civilization.discoveryPath=['eva','spore'];state.civilization.visits.spore=1;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
 state.space.ships=[{id:'dispatch-ui',tier:2,island:'spore',side:'front',food:4,durability:70,reservedBy:null}];fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-ufo="dispatch-ui"]').click();
 await expect(page.locator('.ufo-dialog')).toContainText('调度到露米纳星湾 · 晴昼面');await page.locator('[data-dispatch-ufo="dispatch-ui"]').click();await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships[0].island).toBe('spore');expect(saved.space.ships[0].flight?.kind).toBe('dispatch');expect(saved.space.ships[0].flight?.to.island).toBe('eva');expect(saved.space.ships[0].durability).toBe(70);expect(errors).toEqual([]);
});

test('UFO flight displays a persistent route and animated departure before arrival',async({page})=>{
 const {enqueue}=await import('../src/simulation.js');const state=createGame();state.civilization.technology=120;state.civilization.observations=3;state.civilization.discoveryPath=['eva','spore'];state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
 state.space.ships=[{id:'animated-ufo',tier:2,island:'eva',side:'front',food:8,durability:100,reservedBy:null}];expect(enqueue(state,'voyage','portal',undefined,null,'spore',[],'animated-ufo').ok).toBe(true);
 state.queue[0].phase='acting';state.queue[0].path=[];state.queue[0].elapsed=3;state.speed=0;fixtures.set(page,state);
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await expect(page.locator('.ufo-flight-board')).toContainText('光束吸入');await expect(page.locator('#world')).toHaveAttribute('data-ufo-beams','1');await expect(page.locator('.ufo-flight-board')).toContainText('露米纳星湾 → 童梦星屿');await page.screenshot({path:'artifacts/ufo-takeoff.png'});
