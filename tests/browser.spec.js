...
test('build mode removes an idle UFO and recovers its cargo and rations',async({page})=>{
 const state=createGame();state.speed=0;state.space.ships=[{id:'removable-ufo',tier:2,island:'home',side:'front',food:4,durability:60,reservedBy:null}];state.space.cargo['removable-ufo']=12;state.space.materials.home=8;state.space.provisions.home=3;state.money=100;fixtures.set(page,state);const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5173');await expect(page.locator('#loading')).toBeHidden({timeout:45000});await page.getByRole('button',{name:'探索',exact:true}).click();await page.locator('[data-ufo="removable-ufo"]').click();await expect(page.locator('.ufo-dialog [data-remove-ufo]')).toHaveCount(0);await page.locator('.ufo-dialog .dialog-close').click();await page.locator('#build-button').click();await expect(page.locator('.ufo-build-panel')).toBeVisible();await expect(page.locator('[data-remove-ufo]')).toBeEnabled();page.once('dialog',dialog=>{expect(dialog.message()).toContain('删除');dialog.accept();});await page.locator('[data-remove-ufo]').click();await expect(page.locator('.ufo-build-panel')).toContainText('暂无 UFO');await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships).toHaveLength(0);expect(saved.space.cargo['removable-ufo']).toBeUndefined();expect(saved.space.materials.home).toBe(20);expect(saved.space.provisions.home).toBe(7);expect(saved.money).toBe(520);expect(errors).toEqual([]);
});
...
test('remote UFO can be dispatched from its dialog and dispatch costs one flight use',async({page})=>{
 const state=createGame();state.speed=0;state.civilization.technology=120;state.civilization.observations=3;state.civilization.discoveryPath=['home','spore'];state.civilization.visits.spore=1;state.autonomy.enabled=false;for(const n of Object.values(state.npcs))n.ai.enabled=false;
 ...
 await expect(page.locator('.ufo-dialog')).toContainText('调度到露米纳星湾 · 消耗 1 次航程');await page.locator('[data-dispatch-ufo="dispatch-ui"]').click();await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships[0].island).toBe('home');expect(saved.space.ships[0].durability).toBe(60);expect(errors).toEqual([]);
});
...
test('UFO flight displays a persistent route and animated departure before arrival',async({page})=>{
 ...
 await page.locator('#save').click();const saved=await savedState(page);expect(saved.space.ships).toHaveLength(1);expect(saved.queue[0].elapsed).toBeGreaterThan(0);await page.reload();await expect(page.locator('#loading')).toBeHidden({timeout:45000});await expect(page.locator('.ufo-flight-board')).toContainText('光束放下');await page.locator('[data-speed="1"]').click();await expect(page.locator('.ufo-flight-board')).toBeHidden({timeout:15000});await page.locator('[data-speed="0"]').click();await page.locator('#save').click();const completed=await savedState(page);expect(completed.space.ships[0].island).toBe('home');expect(completed.space.ships[0].durability).toBe(80);expect(errors).toEqual([]);
});
