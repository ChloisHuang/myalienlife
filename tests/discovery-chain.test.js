...
test('authored discovery proceeds from fairytale to ocean without random islands',()=>{
 const g=createGame();g.wonders.archive=3;g.civilization.visits.spore=1;
 for(let i=0;i<3;i++)observe(g,'eva');
 assert.deepEqual(g.civilization.discoveryPath,['eva']);assert.equal(g.civilization.lastDiscoveryObservation,0);
 explore(g,'eva');
 assert.deepEqual(g.civilization.discoveryPath,['eva','spore']);
 observe(g,'spore');
 assert.deepEqual(g.civilization.discoveryPath,['eva','spore']);assert.equal(g.civilization.lastDiscoveryObservation,4);
...
test('exploration stays empty after the last released island',()=>{
 const g=createGame();for(let i=0;i<100;i++)observe(g,'eva');
 assert.deepEqual(g.civilization.discoveryPath,['eva']);assert.equal(Object.keys(g.civilization.islands).length,0);
 g.wonders.archive=3;explore(g,'eva');assert.equal(discovered(g,'city'),false);
...
test('old saves keep all discovered worlds without rolling new ones on reload',()=>{
 ...
 const loaded=restore(serialize(g));assert.deepEqual(loaded.civilization.discoveryPath,['eva','spore']);assert.equal(Object.keys(loaded.civilization.islands).length,0);assert.equal(loaded.version,26);
});
