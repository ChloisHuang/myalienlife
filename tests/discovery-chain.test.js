...
test('old saves keep all discovered worlds without rolling new ones on reload',()=>{
 ...
 const loaded=restore(serialize(g));assert.deepEqual(loaded.civilization.discoveryPath,['home','spore']);assert.equal(Object.keys(loaded.civilization.islands).length,0);assert.equal(loaded.version,25);
});
