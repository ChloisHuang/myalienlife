...
test('v11 exploration migration preserves archive and crystals without retaining legacy city metadata',()=>{
 const g=setup(),o=buyItem(g,'crystal',0,0).object;g.version=11;g.wonders={dust:7,archive:3,lastExpeditionDay:1,expeditions:4,cityRecords:[0,2]};o.wonder={mode:'insight',charge:100,armed:true};const loaded=restore(serialize(g));assert.equal(loaded.version,25);assert.equal(loaded.wonders.archive,3);assert.equal(loaded.wonders.dust,7);assert.equal(loaded.wonders.expeditions,undefined);assert.equal(loaded.wonders.cityRecords,undefined);assert.equal(loaded.objects[0].wonder.armed,true);
});
