...
test('v11 exploration migration preserves archive and crystals without inventing past city visits',()=>{
 const g=setup(),o=buyItem(g,'crystal',0,0).object;g.version=11;g.wonders={dust:7,archive:3,lastExpeditionDay:1};o.wonder={mode:'insight',charge:100,armed:true};const loaded=restore(serialize(g));assert.equal(loaded.version,23);assert.equal(loaded.wonders.archive,3);assert.equal(loaded.wonders.dust,7);assert.equal(loaded.wonders.expeditions,0);assert.equal(loaded.objects[0].wonder.armed,true);
});
