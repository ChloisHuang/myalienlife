...
import {createGame,enqueue,tick,restore,serialize,switchControl,cancelAction,autonomousCandidates} from '../src/simulation.js';
import {createEducation,educationLevel,educationWage,studyInterest,studySubject} from '../src/education.js';
...
test('education scales real wages while keeping adult legacy wages unchanged',()=>{
 for(const [credits,multiplier] of [[0,.65],[6,.75],[18,.85],[36,1],[60,1.2],[96,1.4],[144,1.6]]){
  const g=setup();g.player.age=20;g.player.education.credits=credits;for(const key in g.player.education.foundation)g.player.education.foundation[key]=18;g.career={id:'scientist',level:1,shifts:0};g.skills.science=6;
...
test('learning preferences reflect subject interests and only minors have study candidates',()=>{
 const g=setup();assert.ok(studyInterest(g.player,'music')>studyInterest(g.player,'science'));
 assert.ok(autonomousCandidates(g,'player').some(c=>c.type==='study'));g.player.age=18;assert.ok(!autonomousCandidates(g,'player').some(c=>c.type==='study'));
 g.player.education.credits=144;for(const key in g.player.education.foundation)g.player.education.foundation[key]=18;assert.equal(educationLevel(g.player).name,'博士');assert.equal(createEducation(0).credits,0);
});
