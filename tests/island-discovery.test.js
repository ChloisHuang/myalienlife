...
test('discovery does not generate an island without a released design',()=>{
...
test('each later observation remains undiscovered and does not call random',()=>{
 const g=createGame();g.civilization.discoveryPath=['home','spore'];g.civilization.visits.spore=1;g.civilization.observations=12;
 for(let i=0;i<3;i++){
...
test('saved worlds keep the stopped discovery route after loading',()=>{
 ...
 assert.equal(discoverAdjacentIsland(loaded,'spore'),null);
