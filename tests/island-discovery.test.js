...
test('discovery uses the authored ocean instead of generating a random island',()=>{
 ...
 assert.equal(discoverAdjacentIsland(g,'eva',()=>0).name,'童梦星屿');
...
test('each later observation remains undiscovered and does not call random',()=>{
 const g=createGame();g.civilization.discoveryPath=['eva','spore','ocean'];g.civilization.visits.ocean=1;g.civilization.observations=12;
 for(let i=0;i<3;i++){
...
test('saved worlds retain discovery history and can discover the new authored ocean',()=>{
 const g=createGame();g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;g.civilization.observations=1000;
 const loaded=restore(JSON.stringify(g));
 assert.deepEqual(loaded.civilization.discoveryPath,['eva','spore']);
