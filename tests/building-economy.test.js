...
test('construction consumes one local material per ten seconds without double charging resumed work',()=>{
 ...
 g.space.materials.eva=100;contributeProject(g,'constructIsland',g.player,20);assert.equal(p.construction,10);
...
test('only botanists extract mature plants into local building materials, not a second harvest',()=>{
 ...
 for(let i=0;i<600;i++)tick(g,.1,()=>1);assert.ok(g.space.materials.eva>0);assert.ok(plant.plant.growth<1);assert.equal(g.harvest.spores,0);assert.equal(enqueue(g,'extractMaterials',plant.id).ok,false);
});
test('cargo is capacity limited, local, reserved during flight, and unloads into the destination stock',()=>{
 const g=createGame(),ship={id:'cargo-test',tier:1,island:'eva',side:'front',food:0,durability:100,reservedBy:null};g.space.ships.push(ship);g.space.materials.eva=100;
 assert.equal(loadShipMaterials(g,ship.id,UFOS[0].cargoCapacity+1).ok,false);assert.equal(loadShipMaterials(g,ship.id,10).ok,true);assert.equal(g.space.materials.eva,90);assert.equal(g.space.cargo[ship.id],10);
...
test('an actual voyage carries materials across planets and cancellation never duplicates cargo',()=>{
 const g=createGame();g.autonomy.enabled=false;for(const n of Object.values(g.npcs))n.ai.enabled=false;g.civilization.discoveryPath=['eva','spore'];g.civilization.technology=240;g.space.materials.eva=20;
 const ship={id:'freighter',tier:1,island:'eva',side:'front',food:4,durability:100,reservedBy:null};g.space.ships.push(ship);assert.equal(loadShipMaterials(g,ship.id,20).ok,true);
 assert.equal(enqueue(g,'voyage','portal',undefined,null,'spore',[],ship.id).ok,true);assert.equal(g.space.materials.spore,undefined);
 cancelAction(g,g.queue[0].id);assert.equal(g.space.cargo.freighter,20);assert.equal(g.space.materials.eva,0);assert.equal(enqueue(g,'voyage','portal',undefined,null,'spore',[],ship.id).ok,true);
 const loaded=restore(serialize(g));for(let i=0;i<900;i++)tick(loaded,.1,()=>1);
 assert.equal(loaded.player.island,'spore');assert.equal(loaded.space.materials.spore,20);assert.equal(loaded.space.materials.eva,0);assert.equal(loaded.space.cargo.freighter,undefined);assert.ok(restore(serialize(loaded)));
});
