...
test('stored remote blueprints never replace home furniture or occupants',()=>{
 ...
 g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;addGenerated(g,0,1);addGenerated(g,1);
 ...
 assert.throws(()=>createIslandTerrain(STAR_ISLANDS.eva,new Group()),/固定/);
});
test('generated layouts support arrival, research, saving and return without altering home',()=>{
 ...
 g.civilization.discoveryPath=['eva','spore'];g.civilization.visits.spore=1;addGenerated(g,0,1);g.civilization.technology=240;g.skills.science=18;g.space.ships.push({id:'fixture-ufo',tier:3,island:'eva',side:'front',food:2,durability:100,reservedBy:null});g.skills.botany=18;g.player.preferences.garden=10;
 ...
 const loaded=restore(serialize(g));assert.equal(loaded.player.island,'wild-0');assert.deepEqual(loaded.objects.filter(o=>!o.island||o.island==='eva').map(o=>({id:o.id,x:o.x,z:o.z,rotation:o.rotation})),home.map(o=>({id:o.id,x:o.x,z:o.z,rotation:o.rotation})));
});
...
test('new generation varies building clearings across seeds and supports version one saved outlines',()=>{
 ...
 const old=generateIsland(17,0);old.generationVersion=1;old.decor=old.decor.slice(0,25);const before=structuredClone(old);assert.ok(validIslandBlueprint(old,old.id));const terrain=createIslandTerrain(old,new Group());terrain.dispose();assert.deepEqual(old,before);
});
