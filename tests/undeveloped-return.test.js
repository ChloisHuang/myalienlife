...
function setup(){
 const g=createGame();g.civilization.discoveryPath=['home','spore'];g.civilization.technology=240;
 ...
 g.player.island=g.viewIsland=island.id;g.player.homeIsland='spore';g.player.x=0;g.player.z=2;
...
test('unfinished islands encourage healthy residents to return to their own settled island despite basic facilities',()=>{
 ...
 g.civilization.projects['wild-0'].blueprint=300;g.civilization.projects['wild-0'].construction=600;assert.equal(bonus(g),null);
});
...
test('a resident can autonomously travel from the main island to their settled island while healthy',()=>{
 const g=setup();g.player.island='home';g.player.homeIsland='wild-0';g.objects.push(...[['portal',-5,3],['food',-2,3],['pod',2,3],['shower',5,3]].map(([type,x,z])=>({id:`home-${type}`,type,x,z,island:'home',side:'front',rotation:0})));
 g.space.ships=[{id:'home-ship',tier:3,island:'home',side:'front',food:10,durability:100,reservedBy:null}];
 assert.ok(bonus(g,'wild-0')>0);
...
test('loading interim visit records removes abandoned countdown data without moving residents',()=>{
 const g=setup();g.player.islandVisit={island:'wild-0',arrivedAt:100};g.npcs.nova.islandVisit={island:'home',arrivedAt:100};
 const loaded=restore(serialize(g));assert.equal(loaded.player.islandVisit,undefined);assert.equal(loaded.npcs.nova.islandVisit,undefined);assert.equal(loaded.player.island,'wild-0');assert.equal(loaded.player.homeIsland,'spore');assert.equal(loaded.day,g.day);
});
