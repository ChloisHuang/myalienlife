...
function chain(){
 ...
 g.civilization.discoveryPath=['eva','spore'];
...
test('home is protected and deleting a middle island preserves later islands',()=>{
 const g=chain(),before=serialize(g);assert.equal(destroyIsland(g,'eva').ok,false);assert.equal(serialize(g),before);
...
test('destroying an inhabited island evacuates residents and ships and cancels its work',()=>{
 const g=chain();Object.assign(g.player,{island:'wild-0',settlementIsland:'wild-0'});g.viewIsland='wild-0';
 ...
 assert.equal(destroyIsland(g,'wild-0').ok,true);assert.equal(g.player.island,'eva');assert.equal(g.player.settlementIsland,'eva');assert.equal(g.viewIsland,'eva');assert.equal(g.space.ships[0].island,'eva');
...
test('occupied incubators survive demolition and older saves acquire an empty destruction history',()=>{
 ...
 assert.equal(destroyIsland(g,'wild-0').ok,true);assert.equal(g.incubations.length,1);assert.equal(g.objects.find(o=>o.id==='wild-0-nursery').island,'eva');assert.ok(restore(serialize(g)));
...
test('evacuation does not invalidate an existing population above the remaining island capacity',()=>{
 ...
 for(let i=0;i<8;i++){const id=`evacuee-${i}`;g.npcs[id]={...structuredClone(template),id,uid:id,name:`撤离居民${i}`,island:'wild-0',settlementIsland:'wild-0'};g.relationships[id]=20;}
