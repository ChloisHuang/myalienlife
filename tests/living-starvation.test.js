...
test('tree rest crosses to the other face before searching for a local resting position',()=>{
 const g=quiet();g.player.prayer.nether=10;const tree=buyItem(g,'spiritTree',0,0,0,{island:'eva',side:'back'}).object;
...
test('a previously saved cross-face waiting action resumes movement rather than waiting forever',()=>{
 const g=quiet();g.player.prayer.nether=10;const tree=buyItem(g,'spiritTree',0,0,0,{island:'eva',side:'back'}).object;enqueue(g,'treeRest',tree.id);g.queue[0].phase='waiting';
...
test('critical hunger interrupts optional living activity and pending social invitations to actually eat',()=>{
 ...
 g.queue.push({id:g.nextId++,type:'lounge',targetId:'sofa',target:{x:-3.6,z:0,island:'eva',side:'front'},source:'manual',phase:'walking',elapsed:0,path:null,seat:null,invited:true});
