...
test('ordinary race cannot cross without gates; same-face walking does not blink',()=>{
 const g=fixture();g.player.prayer.nether=0;assert.equal(enqueue(g,'research','back-lab').ok,false);g.player.prayer.nether=10;assert.equal(enqueue(g,'research','lab').ok,true);tick(g,.1);assert.equal(g.queue[0].blinkTransit,undefined);
});
