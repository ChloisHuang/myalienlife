...
test('version 1 saves migrate once without losing household progress or manual orders',()=>{
 ...
 const loaded=sim.restore(JSON.stringify(legacy));assert.equal(loaded.version,23);assert.equal(loaded.money,987);assert.equal(loaded.relationships.nova,65);assert.equal(loaded.autonomy.enabled,false);assert.equal(loaded.queue[0].source,'manual');assert.equal('timer'in loaded.npcs.nova,false);
});
