...
test('school lessons develop foundations, never professional XP, including the university boundary',()=>{
 const p=createGame().npcs.pip,skills={...p.skills};p.education.credits=59;for(const key in p.education.foundation)p.education.foundation[key]=18;
...
test('major is chosen autonomously from existing interests and foundations and stays fixed',()=>{
 const p=createGame().npcs.pip;p.education.credits=60;for(const key in p.education.foundation)p.education.foundation[key]=18;p.preferences={dance:100};chooseMajor(p,()=>.99);assert.equal(p.education.major,'music');
...
test('an adult can continue education and switching control preserves foundation and major',()=>{
 ...
 g.player.education.credits=60;for(const key in g.player.education.foundation)g.player.education.foundation[key]=18;g.player.education.major='science';g.player.education.focus='science';const before=g.skills.science;
 assert.equal(enqueue(g,'study','lab').ok,true);for(let i=0;i<1500&&g.queue.length;i++)tick(g,.1,()=>0);
...
test('all majors retain a nonzero probability even with one overwhelming interest',()=>{
 ...
 for(let i=0;i<1000;i++){const p=createGame().npcs.pip;p.preferences={dance:100};p.education.credits=60;for(const key in p.education.foundation)p.education.foundation[key]=18;chooseMajor(p,()=>i/1000);counts[p.education.major]++;}
...
test('university specialization and foundation XP round-trip exactly',()=>{
 const g=createGame();g.player.education.credits=59;for(const key in g.player.education.foundation)g.player.education.foundation[key]=18;g.player.education.focus='science';completeStudy(g.player,g.skills,'science',()=>0);
