...
const service=await createHttpService({directory:process.env.ORBIT_DATA_DIR??resolve('.data'),dist:process.env.ORBIT_DIST_DIR??resolve('dist'),token,origin:process.env.ORBIT_ORIGIN,release:process.env.ORBIT_RELEASE??'local',trustProxy:true,geoLookup:ip=>geoip.lookup(ip)});
